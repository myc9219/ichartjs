;(function(){
	...
	var KeDu = Jidea.extend(Jidea.Component,{
			...
			doConfig:function(){
				...
				Jidea.Assert.isNumber(this.get('distance'),'distance');
				
				var customLabel = this.get('labels').length>0;
				if(customLabel){
					this.number = this.get('labels').length-1;
				}else{
					Jidea.Assert.isTrue(Jidea.isNumber(this.get('max_scale'))||Jidea.isNumber(this.get('end_scale')),'max_scale&end_scale');
					
					//end_scale must greater than maxScale
					if(!this.get('end_scale')||this.get('end_scale')<this.get('max_scale')){
						this.push('end_scale',Jidea.Math.ceil(this.get('max_scale')));
					}
				...
				for(var i=0;i<=this.number;i++){
					...
					this.items.push(Jidea.merge({text:text,x:x,y:y,textX:x,textY:y},this.fireEvent(this,'parseText',[text,x,y,i])));
	...
	Jidea.Coordinate2D = Jidea.extend(Jidea.Component,{
		configure:function(){
			 ...
			 */
			Jidea.Coordinate2D.superclass.configure.apply(this,arguments);
		...
		doDraw:function(opts){
			...
			if(this.get('alternate_color')){
				var x,y,
					f = false,
					axis =[0,0,0,0],
					c = Jidea.Math.dark(this.get('background_color'),0.04);
		...
		doConfig:function(){
			Jidea.Coordinate2D.superclass.doConfig.call(this);
			Jidea.Assert.isNumber(this.get('width'),'width');
			Jidea.Assert.isNumber(this.get('height'),'height');
			 ...
			 */
			if(this.get('gradient')&&Jidea.isString(this.get('background_color'))){
			...
			if(this.get('crosshair.enable')){
				...
				this.crosshair = new Jidea.CrossHair(this.get('crosshair'),this);
			}
			...
			if(!Jidea.isArray(this.get('kedu'))){
				if(Jidea.isObject(this.get('kedu')))
			...
			if(k2g){
				var kedu,x,y;
 				for(var i=0;i<this.kedu.length;i++){
 					...
 					if(Jidea.isFalse(kedu.get('kedu2grid'))||hg&&kedu.get('which') == 'v'||vg&&kedu.get('which') == 'h'){
 						continue;
		 					}
 			x = y = 0;
			...
			if(vg){
				...
				Jidea.Assert.gtZero(gv['value'],'value');
				var d = w/gv['value'],
					n  = gv['value'];
			...
			if(hg){
				...
				Jidea.Assert.gtZero(gh['value'],'value');
				var d = h/gh['value'],
					n  = gh['value'];
...
Jidea.Coordinate3D = Jidea.extend(Jidea.Coordinate2D,{
		configure:function(){
			 ...
			 * invoked the super class's  configurationuration
			 */
			Jidea.Coordinate3D.superclass.configure.apply(this,arguments);
		...
		doConfig:function(){
			Jidea.Coordinate3D.superclass.doConfig.call(this);
			
			var bg = this.get('background_color'),
				dark_color = Jidea.Math.dark(bg,0.1),
				h = this.get('height'),
				w = this.get('width');
			...
			if(this.get('magical_shell').length==3){
				if(this.get('gradient')){
					...
					lc = Jidea.Math.light(color,0.3);
					this.get('magical_shell')[1].color = this.target.avgLinearGradient(this.x,this.y,this.x,this.y-this.get('yAngle_')*this.get('zHeight'),[color,Jidea.Math.light(color,0.3)]);
