Jidea.CrossHair = Jidea.extend(Jidea.Element,{
		...
		initialize:function(){
			Jidea.CrossHair.superclass.initialize.call(this);
			
			this.top = Jidea.Math.fixPixel(this.get('top'));
			this.left = Jidea.Math.fixPixel(this.get('left'));
			
			this.dom = document.createElement("div");
			this.dom.style.zIndex=this.get('index');
			this.dom.style.position="absolute";
			//set size zero make  integration with vertical and horizontal
			this.dom.style.width= Jidea.Math.toPixel(0);
			this.dom.style.height=Jidea.Math.toPixel(0);
			this.dom.style.top=Jidea.Math.toPixel(this.get('top'));
			this.dom.style.left=Jidea.Math.toPixel(this.get('left'));
			this.css('visibility','hidden');
			
			this.horizontal = document.createElement("div");
			this.vertical = document.createElement("div");
			
			this.horizontal.style.width= Jidea.Math.toPixel(this.get('width'));
			this.horizontal.style.height= Jidea.Math.toPixel(this.get('line_width'));
			this.horizontal.style.backgroundColor = this.get('line_color');
			this.horizontal.style.position="absolute";
			
			this.vertical.style.width= Jidea.Math.toPixel(this.get('line_width'));
			this.vertical.style.height = Jidea.Math.toPixel(this.get('height'));
