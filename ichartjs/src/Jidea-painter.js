...
Jidea.Painter = function(config){
	...
	this.dimension = Jidea.Math._2D;
	/**
	 * define interface method
	 */
	Jidea.DefineAbstract('configure',this);
	Jidea.DefineAbstract('commonDraw',this);
	Jidea.DefineAbstract('initialize',this);
	/**
	 ...
	 * register event
	 */
	if(Jidea.isObject(this.get('listeners'))){
...
Jidea.Painter.prototype = {
	...
	is3D:function(){
		return this.dimension==Jidea.Math._3D;
	},
	draw:function(opts){
		this.init();
		/**
	...
	on:function(name,fn){
		if(Jidea.isString(name)&&Jidea.isFunction(fn))
		this.events[name].push(fn);
		return this;
	},
	doConfig:function(){
		var padding = Jidea.Math.parsePadding(this.get('padding'));
		...
		this.push('fontStyle',Jidea.getFont(this.get('fontweight'),this.get('fontsize'),this.get('font')));
		
		this.push('fill_color',this.get('background_color'));
		this.push("light_color",Jidea.Math.light(this.get('background_color'),this.get('color_factor')));
		this.push("dark_color",Jidea.Math.dark(this.get('background_color'),this.get('color_factor')));
		
		this.push("light_color2",Jidea.Math.light(this.get('background_color'),this.get('color_factor')*2));
		this.push("dark_color2",Jidea.Math.dark(this.get('background_color'),this.get('color_factor'))*2);
	...
	configuration:function(C){
		if(Jidea.isObject(C)){
			Jidea.merge(this.configurations,C);
		}
	},
    /**
