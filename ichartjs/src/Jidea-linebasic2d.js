	...
	Jidea.LineBasic2D = Jidea.extend(Jidea.Line,{
		...
		configure:function(config){
			 ...
			 */
			Jidea.LineBasic2D.superclass.configure.call(this);
		...
		doConfig:function(){
			Jidea.LineBasic2D.superclass.doConfig.call(this);
			...
			for(var i=0;i<this.data.length;i++){
				...
				for(var j=0;j<this.data[i].value.length;j++){
					...
					points.push(Jidea.merge({x:x,y:y,value:this.data[i].value[j]},this.fireEvent(this,'parsePoint',[this.data[i].value[j],x,y,j])));
				}
				...
				this.lines.push(new Jidea.LineSegment(this.get('segment_style'),this));
			}
