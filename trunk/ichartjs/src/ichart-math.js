...
var sin = Math.sin, cos = Math.cos, tan = Math.tan,
		...
		iChart.Math = {
			...
			atanToAngle:function(ox,oy,x,y){
				...
				var quadrant = iChart.Math.quadrant(ox,oy,x,y);
				var angle = iChart.Math.radianToAngle(Math.atan(Math.abs((oy-y)/(ox-x))));
				if(quadrant==1){
					angle = 180 - angle;
				}else if(quadrant==2){
					angle = 180 + angle;
			...
			between:function(low,up,value){
				return iChart.Math.lowTo(low,iChart.Math.upTo(up,value));
			},
...
//				var L = Math.sin(iChart.Math.angleToRadian(y)),
//					A = iChart.Math.angleToRadian(x);
//				return {
//					x:L*Math.sin(A),
//					y:L*Math.cos(A)
//				}
				if(!radian){
					y = iChart.Math.angleToRadian(y);
					x = iChart.Math.angleToRadian(x);
				}
				y = Math.sin(y);
				return {
					x:y*Math.sin(x),
			...
			fixPixel: function(v) {
				return iChart.isNumber(v)?v:parseFloat(v.replace('px',""))||0 ;
			},
			toPixel: function(v) {
				return iChart.isNumber(v)?v+'px':iChart.Math.fixPixel(v)+'px';
			}
		}
})();